...
const AddOrderModal: React.FC<AddOrderModalProps> = ({ isOpen, onClose, onSaved, editingOrder, onYearChange }) => {
  ...
  useEffect(() => {
    const shouldAutoCalculate = !isAmountManuallyEdited;
  ...
  const handleSubmit = async () => {
    ...
    if (!formData.payment_amount || formData.payment_amount === '0') {
      alert('請先確認表單中的總金額，系統會以目前畫面上的金額寫入資料庫。');
      return;
    }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-5">
              ...
              <div>
                ...
                {editingOrder && (
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">儲存時會以目前表單中的總金額寫入資料庫；若要恢復系統計算，請使用「恢復原計算」。</p>
                )}
              </div>
