...
const ShuttleImagesPage: React.FC = () => {
  ...
  const fetchImages = async () => {
    ...
    setLoading(true);
    try {
      const response = await shuttleImagesApi.list({ store_id: currentStore.id });
      ...
      if (hasDuplicates && fetchedImages.length > 0) {
        ...
        // 重新獲取列表
        const updatedResponse = await shuttleImagesApi.list({ store_id: currentStore.id });
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      await shuttleImagesApi.create(imageFile, finalSortOrder, currentStore.id);
      await fetchImages();
      handleRemovePreview();
    } catch (error: any) {
