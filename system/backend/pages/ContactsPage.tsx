...
import { Search, Edit2, Trash2, X, Loader2, MessageCircle, Phone, CheckCircle, XCircle, Clock } from 'lucide-react';
...
const ContactsPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingContact) {
        await contactsApi.update(editingContact.id, submitData);
      }
  ...
  return (
    <div className="px-6 pb-6 pt-0 dark:text-gray-100">
      <div className="mb-8 flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100">聯絡管理</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">管理聯絡表單資料</p>
        </div>
      </div>
      ...
      <div className="mb-6 flex gap-4">
        <div className="flex-1 relative">
          ...
          <input
            type="text"
            placeholder="搜尋姓名、LINE ID、電話、訊息內容..."
            value={searchTerm}
      ...
      {contacts.length === 0 ? (
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-12 text-center">
          <p className="text-gray-500 dark:text-gray-400">目前沒有聯絡資料</p>
        </div>
      ) : (
            ...
            <table className="w-full text-left">
              <thead className="bg-gray-50 dark:bg-gray-800/50 border-b border-gray-200 dark:border-gray-700">
                <tr>
                  ...
                  <th className="px-6 py-4 text-sm font-bold text-gray-700 dark:text-gray-300">訊息內容</th>
                ...
                {contacts.map((contact) => (
                  <tr key={contact.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                    <td className="px-6 py-4">
                      <span className="text-sm font-medium text-gray-800 dark:text-gray-100">{contact.name}</span>
                    </td>
                    <td className="px-6 py-4">
                      <span className="text-sm text-gray-600 dark:text-gray-400">{contact.line_id}</span>
                    </td>
                    <td className="px-6 py-4">
                      <span className="text-sm text-gray-600 dark:text-gray-400">{contact.phone || '-'}</span>
                    </td>
                    <td className="px-6 py-4">
                      <span className="text-sm text-gray-600 dark:text-gray-400 line-clamp-2" title={contact.message}>
                        {contact.message}
                      </span>
                    </td>
                    <td className="px-6 py-4">
