        ...
        <div className="p-6 space-y-6 overflow-y-auto max-h-[70vh]">
          <div className="grid grid-cols-2 gap-4">
             <div className="bg-blue-50 p-4 rounded-xl border border-blue-100">
                ...
                <p className="text-2xl font-black text-blue-800">${stats.total_amount.toLocaleString()}</p>
             </div>
             <div className="bg-orange-50 p-4 rounded-xl border border-orange-100">
                <p className="text-xs text-orange-600 font-bold mb-1">全平台總車次</p>
                <p className="text-2xl font-black text-orange-800">{stats.total_count} 台</p>
             </div>
          </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
              ...
              <tbody className="divide-y divide-gray-100">
                {orders.map((order) => (
                  <tr key={order.id} className="hover:bg-gray-50/50 group transition-colors">
                    ...
                    <td className="px-4 py-4 text-gray-500">{formatDate(order.appointment_date)}</td>
                    <td className="px-4 py-4 text-gray-500">{formatDateTime(order.start_time)}</td>
                    <td className="px-4 py-4 text-gray-500">{formatDateTime(order.end_time)}</td>
                    <td className="px-4 py-4 text-gray-500 font-bold">{formatDateTime(order.expected_return_time)}</td>
                    <td className="px-4 py-4">
                    ...
                    <td className="px-4 py-4 text-orange-600 font-bold">{order.partner?.name || '-'}</td>
                    <td className="px-4 py-4">
                      <div className="text-xs text-gray-400 mb-0.5">{order.payment_method || '-'}</div>
                      <div className="font-black text-gray-900">${order.payment_amount.toLocaleString()}</div>
                    </td>
        ...
        <div className="p-4 border-t border-gray-100 flex items-center justify-between bg-gray-50/30">
          <div className="text-sm text-gray-500 font-medium">
            本月總計: <span className="font-bold text-orange-600">{stats?.total_count || 0} 台</span>, 
            總金額: <span className="font-bold text-green-600">${(stats?.total_amount || 0).toLocaleString()}</span>
          </div>
          <div className="flex items-center space-x-2">
            <button 
              onClick={() => setCurrentPage(p => Math.max(1, p - 1))}
              disabled={currentPage === 1}
              className="p-2.5 border border-gray-200 rounded-xl hover:bg-white disabled:opacity-30 transition-all shadow-sm"
            >
              <ChevronLeft size={16} />
            </button>
