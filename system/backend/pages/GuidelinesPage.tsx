...
const GuidelinesPage: React.FC = () => {
  ...
  const handleOpenModal = (guideline?: Guideline) => {
    if (guideline) {
      ...
      const fixedStoreId = guideline.store_id?.toString() || currentStore?.id.toString() || '';
      setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const fixedStoreId = editingGuideline?.store_id || currentStore.id;
      const submitData = {
        ...formData,
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              <div>
                ...
                <input
                  type="text"
                  value={editingGuideline?.store?.name || currentStore?.name || '未指定'}
