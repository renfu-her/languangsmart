...
## 2026-01-11 08:35:00 - 修改所需租車類型/數量的顯示格式並添加顏色標示
...
- **OrdersPage** (`system/backend/pages/OrdersPage.tsx`)
  - 修改「所需租車類型/數量」的顯示方式：
    - 改為簡單的文字顯示，但每個車型使用不同顏色標示
    - 格式：機車型號 x 數量，機車型號 x 數量（用逗號分隔）
    - 例如：EB-500 電輔車 x 1，ES-2000 白牌 x 1
    - 每個車型根據索引使用不同顏色（藍色、綠色、橙色、紫色等）
  - 保留價格計算邏輯，不影響原來的計價方式

### 功能說明
- 「所需租車類型/數量」現在以簡單的文字格式顯示，但每個車型有顏色標示
- 多個車型用逗號分隔
- 價格計算邏輯保持不變
