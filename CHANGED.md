...
## 2026-04-24 - 月報表新增扣除已結金額

- 在合作商機車出租月報表底部新增「已結金額」與「需結總金額」。
- 「需結總金額」計算為總金額扣除紅字已結清訂單加總，方便對帳未結餘額。
- 已結金額沒有資料時顯示 `0`，並將紅字標示套用在「需結總金額」而非「總金額」。
- 將新增的兩個金額列套用黃色總計列樣式，確保匯出後直接顯示在總金額下方。
- 同步修正後台合作商單月詳細統計彈窗的前端 Excel 匯出，讓 Export 按鈕產出的檔案也包含兩個金額列。
- 修正月報表匯出天數與金額小數問題，改以日期差整數天計算並輸出整數金額。
